...
renderTemplate(app, async () => {
  const model = await loadModel(
    {
      modelLibWasmUrl:
        "http://localhost:5173/lib/Llama-3-8B-Instruct-q3f16_1.wasm",
      modelWeightsConfigUrl:
        "https://huggingface.co/mlc-ai/Llama-3-8B-Instruct-q3f16_1-MLC/resolve/main/",
    },
    (report) =>
      (app.innerHTML = `<pre id='progress'><code>${JSON.stringify(report, null, 2)}</code></pre>`),
